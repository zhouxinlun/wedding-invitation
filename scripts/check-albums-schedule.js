...
check('原封面文件保持不变，照片不会在主页面引用分包资源',()=>{
 const hash=crypto.createHash('sha256').update(fs.readFileSync(path.join(root,'assets/couple-red.jpg'))).digest('hex');assert.equal(hash,'156621da9b617f770ae20e743ac9608b3c25401ce0a1105f8f2dad7c777ed4ac');
...
check('两家小区有效定位、流程地点引用及时间顺序正确',()=>{
 ...
 assert.equal(wedding.guestArrivalTime,'10:30');assert.equal(wedding.ceremonyTime,'10:58');assert.equal(wedding.schedule.at(-1).events.at(-1).time,wedding.ceremonyTime);assert(wedding.preparationNotes.some(n=>n.includes('包间')&&n.includes('确认')));
 const calendar=fs.readFileSync(path.join(root,'../exports/婚礼日程.ics'),'utf8').replace(/\r\n /g,'');assert(calendar.includes('10:30'));assert(calendar.includes(wedding.venue.room));
});
