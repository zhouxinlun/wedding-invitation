...
function setup(webFile){
  ...
  let paused=true,reject=false,resolvePlay=null;const calls=[];
  ...
  w.eval(source);
  return {w,calls,audio:w.document.querySelector('audio'),button:w.document.querySelector('.wedding-music'),badge:w.document.querySelector('[data-music-toggle]'),set reject(value){reject=value;},set delay(value){resolvePlay=value;},close:()=>w.close()};
}
(async()=>{
