...
(async()=>{
  ...
  for(const {hash,interacted=false} of [{hash:''},{hash:'#us'},{hash:'#invitation'},{hash:'#blessings'},{hash:'',interacted:true}]){
    ...
    try{
      ...
      const books=[...doc.querySelectorAll('.album-book')];
      assert(books.every(book=>!book.querySelector('img').hasAttribute('src')),'No album cover fetches on initial page construction');
      const bookObserver=observers.find(observer=>observer.targets.includes(books[0]));
      bookObserver.callback([{isIntersecting:true,target:books[0]}]);
      assert(books[0].querySelector('img').hasAttribute('src'),'Approaching a book loads its cover');
      assert(books.slice(1).every(book=>!book.querySelector('img').hasAttribute('src')),'Unseen albums stay deferred');
